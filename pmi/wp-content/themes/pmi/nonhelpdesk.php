<?php
/*
Template Name: NON HelpDisk Template
*/
?>
<?php get_header();?>
<?php
if(isset($_POST['nonsubmit_help']))
{  
	$nonhelp = nonhelpdisk($_POST);
}
?>
 <script>
$(document).ready(function() {
	   $(".thumb").hover(function() {
		...
		<?php if(isset($nonhelp) && !empty($nonhelp)){?>
		$(".success-msg").text("<?php echo $nonhelp; ?>");
		$("#myModal2").modal('show');
		setTimeout(function(){
		   $(".close").trigger("click");
			 }, 5000);
...
<p>The Help Desk is managed by a set of dedicated Volunteers with day jobs. They will get back to you as soon as possible..
              ...
              <form method="post" action="" name="hiring_form" enctype="multipart/form-data">
                    ...
                    <p><label>First Name</label><input type="text"   name="help_name"  /></p>
                    <p><label>Last Name</label><input type="text" name="last_name" ></p>
                    <p><label>Email Address</label><input type="text"   name="help_mail"/></p>
                  ...
                  <p><span><input type="submit" name="nonsubmit_help"  value="Submit"  class="btn_search_carrer"/></span><span><input type="reset" name="reset" value="Clear"></span></p>
              </form>
